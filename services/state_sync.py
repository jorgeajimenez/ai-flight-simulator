...
class PersistentWorldClient:
    """
    Persistent World State Sync powered by Cloud Firestore and Cloud Storage.
    ...
    def get_recent_activity(lat: float, lon: float) -> list:
        ...
        try:
            ...
            for doc in docs:
                data = doc.to_dict()
                results.append(
                    f"Anomaly: '{data.get('prompt')}' detected at Lat {data.get('latitude'):.2f}, Lon {data.get('longitude'):.2f}"
                )

            return results
        except Exception as e:
            logger.error(f"State Sync Read Error: {e}")
            return ["Unable to scan Persistent World database."]
    ...
    def log_terraform_event(lat: float, lon: float, prompt: str, image_b64: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"State Sync: Creating new storage bucket '{bucket_name}'..."
                )
                bucket = storage_client.create_bucket(
                    bucket_name, location=GCPConfig.LOCATION
                )
