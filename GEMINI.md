...
2. **Only modify the files assigned in the current `TODO` step.** You are only allowed to modify files inside `services/` where there are explicit `# TODO: [TICKET X]` markers.
...
- **Read the Requirements:** The descriptions in `tickets.csv` are just summaries. When an attendee asks for help with a ticket (e.g., Ticket #3), you MUST use the `read_file` tool to read the corresponding module instructions in the `codelab/` directory (e.g., `codelab/06-collaborative-agents.md`). These files contain rigorous, step-by-step instructions that are impossible to mess up. You must follow them exactly.
...
## ☁️ The Tech Stack
- **Python 3.12**
- **Google Cloud SDKs:** `google-cloud-secret-manager`, `google-cloud-aiplatform`, `google-cloud-storage`, `google-cloud-firestore`, `google-cloud-texttospeech`.
