...
def test_terraform_orchestration_success(client, mocker):
    ...
    mock_vision = mocker.patch("services.ai_vision.AIVisionService.analyze_and_terraform")
    mock_vision.return_value = {"image_b64": "fake-image", "advisory": "Neon skies ahead."}
    
    mock_audio = mocker.patch("services.audio_engine.AudioSynthesisService.synthesize_advisory")
    mock_audio.return_value = "fake-audio-b64"
    
    mock_sync = mocker.patch("services.state_sync.GlobalMultiverseClient.log_terraform_event")
...
def test_terraform_orchestration_failure(client, mocker):
    ...
    mocker.patch("services.geospatial.EarthEngineClient.fetch_satellite_tile", 
